// utils.js
import { UI_TEXT, CLOUDINARY_BASE_URL } from './core/config.js';
import { state } from './core/state.js';

// Funzione Traduzione (t)
export function t(key) {
    // Fallback sicuro se la lingua o la chiave non esistono
    const langDict = UI_TEXT[state.currentLang] || UI_TEXT['it'];
    return langDict[key] || key; 
}

// Funzione recupero colonna DB multilingua (dbCol)
...
// Verifica Festività Italiana
export function isItalianHoliday(dateObj) {
    const d = dateObj.getDate();
    const m = dateObj.getMonth() + 1; 
    const y = dateObj.getFullYear();

    if (dateObj.getDay() === 0) return true; // Domenica
