...
function renderHome() {
    ...
    <div id="mascot-container" style="position: fixed; bottom: 70px; right: 20px; z-index: 2147483647; display: flex; flex-direction: column; align-items: flex-end; pointer-events: none;">
        ...
        <div onclick="window.toggleChicco()" style="cursor: pointer; transition: transform 0.2s; pointer-events: auto;">
            <img src="${chiccoImg}" 
                 style="width: 100px; height: auto; filter: drop-shadow(0 4px 5px rgba(0,0,0,0.3)); display: block;" 
                 alt="Chicco"
...
window.toggleChicco = async function() {
    console.log("🍇 Click rilevato su Chicco!"); // DEBUG

    const bubble = document.getElementById('chicco-bubble');
    const textSpan = document.getElementById('chicco-text');
    
    // Controllo errori
    if (!bubble) { console.error("❌ ERRORE: Elemento 'chicco-bubble' non trovato."); return; }
    if (!textSpan) { console.error("❌ ERRORE: Elemento 'chicco-text' non trovato."); return; }

    // SE È CHIUSO -> APRI
    if (bubble.style.display === 'none' || bubble.style.display === '') {
        console.log("🔹 Apro il fumetto...");
        bubble.style.display = 'block';
        
        textSpan.innerHTML = `Mmh... <span class="material-icons spin" style="font-size:0.9rem;">sync</span>`;
        
        // Chiama data-logic.js
        if (window.getChiccoRealTimeAdvice) {
            console.log("🔹 Chiedo consiglio al cervello...");
            ...
            console.warn("⚠️ getChiccoRealTimeAdvice non trovato!");
            textSpan.innerHTML = "Ciao! (Errore Data Logic)";
        }
    } 
    // SE È APERTO -> CHIUDI
    else {
...
window.viaggiaConChicco = function(viewName, targetId) {
    ...
    if (targetId && targetId !== 'undefined') {
        ...
        setTimeout(() => { window.openModal(viewName, targetId); }, 100);
        return;
    }
