// ui-renderers.js
// Path imports corretti per file nella ROOT
import { t, dbCol, getSmartUrl, registerItem } from './core/utils.js';
import { state } from './core/state.js';

// ... (Il resto del codice rimane identico, copia tutto il contenuto delle funzioni renderer dal tuo file originale qui sotto) ...

const safeJson = (obj) => encodeURIComponent(JSON.stringify(obj)).replace(/'/g, "%27");

export const ristoranteRenderer = (r) => {
    const uniqueId = registerItem(r, 'rest'); 
    ...
    return `<div class="restaurant-glass-card"><h3 class="rest-card-title">${nome}</h3><p class="rest-card-subtitle"><span class="material-icons">restaurant</span>${paesi}</p><div class="rest-card-actions"><div class="action-btn btn-info rest-btn-size" onclick="openModal('ristorante', '${uniqueId}')"><span class="material-icons">info_outline</span></div>${numero ? `<div class="action-btn btn-call rest-btn-size" onclick="window.location.href='tel:${numero}'"><span class="material-icons">call</span></div>` : ''}<div class="action-btn btn-map rest-btn-size" onclick="window.open('${mapLink}', '_blank')"><span class="material-icons">map</span></div></div></div>`;
};
...
export const sentieroRenderer = (s) => {
    ...
    <div class="card-sentiero-modern animate-fade">
        ...
        <div class="sentiero-card-overlay" 
             style="cursor: pointer;" 
             onclick="openModal('trail', '${uniqueId}')">
            
            <h2 class="sentiero-overlay-title">${titoloMostrato}</h2>
...
export const prodottoRenderer = (p) => {
    const uniqueId = registerItem(p, 'prod'); 
    ...
    return `<div class="culture-card is-product animate-fade" onclick="openModal('product', '${uniqueId}')"><div class="culture-info"><div class="culture-title">${titolo}</div><div class="product-subtitle"><span class="material-icons">stars</span> ${t('ideal_for')}: ${ideale}</div></div><div class="culture-product-thumb"><img src="${imgUrl}" loading="lazy" alt="${titolo}"></div></div>`;
};

export const trainSearchRenderer = (data, nowTime) => {
    return `<div class="bus-search-box animate-fade" style="border-top: 4px solid #c0392b; background: rgba(0,0,0,0.6); backdrop-filter: blur(10px); color: white; border-radius: 20px;"><div class="bus-title"><span class="material-icons" style="background: rgba(255, 255, 255, 0.3); color:#e74c3c;">train</span> Cinque Terre Express</div><div style="padding: 0 5px;"><p style="font-size:0.9rem; color:#ccc; line-height:1.5; margin-bottom:15px;">${t('train_desc')}</p><div style="background: rgba(255, 255, 255, 0.19); border-radius:12px; padding:15px; margin-bottom:20px; border:1px solid rgba(255,255,255,0.1);"><h4 style="margin:0 0 10px 0; font-size:0.8rem; color:#aaa; text-transform:uppercase;">⏱️ ${t('avg_times')}</h4><div style="display:flex; justify-content:space-between; border-bottom:1px solid rgba(255,255,255,0.1); padding:8px 0;"><span>La Spezia ↔ Riomaggiore</span> <b style="color:white;">7 min</b></div><div style="display:flex; justify-content:space-between; border-bottom:1px solid rgba(255, 255, 255, 0.25); padding:8px 0;"><span>${t('between_villages')}</span> <b style="color:white;">2-4 min</b></div><div style="display:flex; justify-content:space-between; padding:8px 0;"><span>Monterosso ↔ Levanto</span> <b style="color:white;">5 min</b></div></div></div><button onclick="apriTrenitalia()" class="btn-yellow" style="background: #c0392b; color: white; border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 4px 15px rgba(192, 57, 43, 0.4); display:flex; align-items:center; justify-content:center; gap:10px; width:100%; padding:15px; border-radius:12px;"><span class="material-icons" style="font-size:1.2rem;">confirmation_number</span> <span style="font-weight:bold;">${t('train_cta')}</span></button><p style="font-size:0.75rem; text-align:center; color:#888; margin-top:10px;">${t('check_site')}</p></div>`;
};